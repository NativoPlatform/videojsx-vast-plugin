...
var IVPAIDAdUnit = __webpack_require__(596);
var Subscriber = __webpack_require__(437);
...
var VPAIDAdUnit = __webpack_require__(13);
...
;// CONCATENATED MODULE: external "videojs"
const external_videojs_namespaceObject = videojs;
var external_videojs_default = /*#__PURE__*/__webpack_require__.n(external_videojs_namespaceObject);
// EXTERNAL MODULE: ./node_modules/@dailymotion/vast-client/dist/vast-client.min.js
var vast_client_min = __webpack_require__(248);
...
class UI extends (external_videojs_default()).EventTarget {
  ...
  setUp() {
    ...
    const setupBlocker = () => {
      const blocker = this.blocker = window_default().document.createElement('div');
    ...
    const setupSkipButton = () => {
      const skipButtonElement = this.skipButtonElement = window_default().document.createElement('div');
      ...
      skipButtonElement.onclick = e => {
        ...
        if ((window_default()).Event.prototype.stopPropagation !== undefined) {
          e.stopPropagation();
        } else {
          return false;
        }
...
;// CONCATENATED MODULE: ./src/tracked-ad.js
class TrackedAd {
  #linearAdTracker;
  #companionTracker;
  #skipAfterDuration;
...
;// CONCATENATED MODULE: ./src/ad-loader.js
...
class AdLoader {
  ...
  loadAdsWithXml(xml) {
    return new Promise((accept, reject) => {
      let xmlDocument;
      if (xml.constructor === (window_default()).XMLDocument) {
        xmlDocument = xml;
      } else if (xml.constructor === String) {
        xmlDocument = new (window_default()).DOMParser().parseFromString(xml, 'text/xml');
      } else {
  ...
  #createTrackedAds = ads => {
    const createTrackedAd = ad => {
      const linearAdTracker = new vast_client_min.VASTTracker(this.#vastClient, ad, ad.creatives.find(linearFn), ad.creatives.find(companionFn));
...
function onClickThrough(url) {
  window_default().open(url, '_blank');
}
;// CONCATENATED MODULE: ./src/ad-selector.js
...
var VPAIDHTML5Client = __webpack_require__(178);
...
;// CONCATENATED MODULE: ./src/vpaid-handler.js
...
class VPAIDHandler {
  ...
  handle(tracker) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const adUnitLoad = (err, adUnit) => {
        ...
        const onHandShake = (error, version) => {
          ...
          if (videoInstance === 'same') {
            ...
            videoElement = window_default().document.createElement('video');
          ...
          (url, id, playerHandles) => {
            if (playerHandles) {
              tracker.once('clickthrough', resolvedUrl => {
                window_default().open(resolvedUrl, '_blank');
              });
            }
      ...
      const vpaidClient = new (VPAIDHTML5Client_default())(container, techScreen, {});
...
function createHtmlContainer(options) {
  ...
  const vpaidContainerElement = document_default().createElement('div');
  if (containerId) {
...
function findHtmlContainer(options) {
  const containerId = options.vpaid.containerId;
  const containerClass = options.vpaid.containerClass;
  let vpaidContainerElement = document_default().getElementById(containerId);
  if (!vpaidContainerElement) {
    vpaidContainerElement = document_default().getElementsByClassName(containerClass)[0];
  }
  return vpaidContainerElement;
}
...
;// CONCATENATED MODULE: ./src/vast-plugin.js
...
const Plugin = external_videojs_default().getPlugin('plugin');
...
class VastPlugin extends Plugin {
  ...
  constructor(player, options) {
    ...
    options = external_videojs_default().mergeOptions(DEFAULT_OPTIONS, options || {});
    ...
    const showCompanionAd = () => {
      ...
      const dest = document_default().getElementById(options.companion.elementId);
      if (companionTracker && companionTracker.variation && dest) {
        const variation = companionTracker.variation;
        const onClick = () => {
          companionTracker.click();
        };
        const hyperLink = document_default().createElement('a');
        hyperLink.src = '#';
        hyperLink.addEventListener('click', onClick);
        const image = document_default().createElement('img');
...
external_videojs_default().registerPlugin('vast', VastPlugin);
})();
